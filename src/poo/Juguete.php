...
class Juguete 
{
    ...
    public function TraerTodos(Request $request, Response $response, array $args): Response
    {
        ...
        if (count($juguetes)) {
            $obj_respuesta->exito = true;
            $obj_respuesta->mensaje = "Juguetes ok";
    ...
    public function BorrarUno(Request $request, Response $response, array $args): Response
    {
        $obj_respuesta = new stdclass();
        $obj_respuesta->exito = false;
        $obj_respuesta->mensaje = "No se pudo eliminar el juguete";
        $obj_respuesta->status = 418;
        ...
        if (
            ...
            // 1) propietario- 2) encargado - 3) empleado

            if ($perfil_usuario == "propietario") {
                if (Juguete::BorrarJuguete($id)) {
                    $obj_respuesta->exito = true;
                    $obj_respuesta->mensaje = "Juguete eliminado correctamente";
                    $obj_respuesta->status = 200;
                } else {
                    $obj_respuesta->mensaje = "El juguete solicitado no existe en el listado!";
                }
            } else {
                $obj_respuesta->mensaje = "No permitido. El usuario debe ser propietario para realizar esta accion. {$usuario_token->nombre} - {$usuario_token->apellido} - {$usuario_token->perfil}";
            }
        }
    ...
    public function ModificarUno(Request $request, Response $response, array $args): Response
    {
        ...
        $obj_respuesta = new stdclass();
        $obj_respuesta->exito = false;
        $obj_respuesta->mensaje = "El juguete no pudo ser modificado";
        $obj_respuesta->status = 418;
        ...
        if (
            ...
            if ($perfil_usuario == "supervisor") {
                ...
                $obj_respuesta->mensaje = "No permitido. El usuario debe ser supervisor para realizar esta accion. {$usuario_token->nombre} - {$usuario_token->apellido} - {$usuario_token->perfil}";
            }
        }
    ...
    public function ListarPdf(Request $request, Response $response, array $args): Response
    {
        ...
        if (isset($request->getHeader("token")[0])) {
            ...
            $mpdf->SetHeader('Barbara Barbara||{PAGENO}{nbpg}');
